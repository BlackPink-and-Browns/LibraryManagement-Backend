...
class AuthorController {
  constructor(private authorService: AuthorService, router: Router) {
    router.post(
      "/",
      checkRole([EmployeeRole.ADMIN, EmployeeRole.HR]),
      this.createAuthor.bind(this)
    );
  ...
  async getAllAuthors(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      next(error); // Passes error to the global error handler middleware
    }
  }

  async getAuthorByID(req: Request, res: Response, next: NextFunction) {
    try {
      const e = await this.authorService.getAuthorByID(Number(req.params.id));
      if (!e) {
  ...
  updateAuthor = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await this.authorService.updateAuthor(id, updateAuthorDTO, req.user?.id);
